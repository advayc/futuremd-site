    ...
    <main className="min-h-screen pt-8 dark:bg-dark-bg bg-light-bg transition-colors duration-700">
      ...
      <header className="pt-4 px-4 w-full max-w-7xl mx-auto text-center relative">
        <button
          ...
          className="absolute top-4 left-4 text-dark-text hover:text-default-900 transition-colors"
        >
          &#60; Back to Blog
        </button>

        <h1 className="text-4xl md:text-6xl font-bold my-8 mb-4 text-center dark:text-white text-black">{post.title}</h1>
        <h2 className="text-center text-lg md:text-xl font-semibold dark:text-dark-text text-dark-text">
          {post.description}
        </h2>
        ...
        <div className="mb-8 flex w-full flex-col items-start">
          <div className="flex items-center space-x-3 rounded rounded-lg cursor-pointer transition-colors px-4 py-1.5 hover:bg-default-100 dark:hover:bg-default-50 transition-colors -ml-4">
            <img className="w-11 h-11 rounded-full" src={post.author.avatar} alt={post.author.name} />
            <div>
              <p className="text-gray-200 font-semibold">{post.author.name}</p>
              <a href={post.author.link} className="text-zinc-500 text-sm hover:text-hov transition-colors">
                @{post.author.user}
              </a>
            </div>
          </div>
